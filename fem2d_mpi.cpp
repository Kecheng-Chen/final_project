...
void Send_data(int my_rank,int p,int ib,int NODE_NUM,double a[]);
void Gather_data(int my_rank,int p,int ib,int NODE_NUM,double a[]);
...
void adjust_boundary ( int node_num, double node_xy[], int node_boundary[],
                       int ib, double time, double a[], double f[],int my_rank, int p);
...
void assemble_adjust ( int node_num, double node_xy[], int nnodes,
                       ...
                       int ib, double time, double a[], double f[], double time_step_size, double u_old[],int my_rank, int p);
int bandwidth ( int nnodes, int element_num, int element_node[],
                int node_num );
void compare ( int node_num, double node_xy[], double time, double u[] );
int dgb_fa ( int n, int ml, int mu, double a[], int pivot[],int my_rank, int p);
...
//
    ...
    for ( time_step = 1; time_step <= time_step_num; time_step++ )
    {
        ...
        assemble_adjust ( NODE_NUM, node_xy, NNODES,
                          ...
                          wq, xq, yq, element_area, ib, time, a, f, time_step_size, u_old,my_rank,p);
        adjust_boundary ( NODE_NUM, node_xy, node_boundary, ib, time, a, f,my_rank, p);
        MPI_Barrier(MPI_COMM_WORLD);
        ierr = dgb_fa ( NODE_NUM, ib, ib, a, pivot, my_rank, p);
        Gather_data(my_rank, p, ib, NODE_NUM, a);
        if (my_rank == 0){
            job = 0;
...
void Send_data(int my_rank,int p,int ib,int NODE_NUM,double a[]) {
    ...
    if (my_rank == 0){
        ...
        for (i = 0; i < NODE_NUM; i++) {
            dest = i%p;
            if (dest != 0) {
                MPI_Send(&a[(i)*col], col, MPI_DOUBLE, dest, i, MPI_COMM_WORLD);
            }
        }
    } else {
        int i;
        for (i = 0; i < NODE_NUM; i++) {
            dest = i%p;
            if (dest == my_rank) {
                MPI_Recv(&a[(i)*col], col, MPI_DOUBLE, source, i, MPI_COMM_WORLD, &status);
            }
        }
    }
}
...
void assemble_adjust ( int node_num, double node_xy[], int nnodes, int element_num,
                       ...
                       double element_area[], int ib, double time, double a[], double f[], double time_step_size, double u_old[],int my_rank, int p)
{
    double aij;
    int basis;
    double bi;
...
int dgb_fa ( int n, int ml, int mu, double a[], int pivot[],int my_rank, int p)
{
    ...
    for ( k = 1; k <= n-1; k++ )
    {
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Bcast(&pivot[k-1], 1, MPI_INT, (k-1)%p, MPI_COMM_WORLD);
        ...
        MPI_Bcast(&a[m+1-1+(k-1)*col], lm, MPI_DOUBLE, (k-1)%p, MPI_COMM_WORLD);
        for (j = k+1; j <= ju; j++)
        {
            l = l - 1;
            mm = mm - 1;
